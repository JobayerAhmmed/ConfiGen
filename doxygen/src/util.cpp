...
/**
 * Creates Used at text with line and file links.
 * @author Jobayer Ahmmed
 */
std::stringstream getUsedAt(QCString sourceFileName, QCString fileName, IntVector lineNr, bool isSourceBrowser)
{
  std::stringstream ss;
  const int maxAnchorStrLen = 20;
  char anchorStr[maxAnchorStrLen];
  int size = lineNr.size();

  if (isSourceBrowser) {
    if (size == 1) {
      ss << "Used at line ";
      anchorStr[0]='\0';
      qsnprintf(anchorStr, maxAnchorStrLen, "l%05d", lineNr.front());
      ss << "<a class=\"el\" href=\"" << sourceFileName << ".html#" << anchorStr << "\">" << lineNr.front() << "</a>";
    } else {
      ss << "Used at lines ";
      if (size == 2) {
        anchorStr[0]='\0';
        ...
        ss << " and ";
        anchorStr[0]='\0';
        qsnprintf(anchorStr, maxAnchorStrLen, "l%05d", lineNr.back());
        ss << "<a class=\"el\" href=\"" << sourceFileName << ".html#" << anchorStr << "\">" << lineNr.back() << "</a>";
      } else {
        anchorStr[0]='\0';
        qsnprintf(anchorStr, maxAnchorStrLen, "l%05d", lineNr.front());
        ss << "<a class=\"el\" href=\"" << sourceFileName << ".html#" << anchorStr << "\">" << lineNr.front() << "</a>";
        for (int i = 1; i < size - 1; i++) {
          anchorStr[0]='\0';
          qsnprintf(anchorStr, maxAnchorStrLen, "l%05d", lineNr[i]);
          ss << ", <a class=\"el\" href=\"" << sourceFileName << ".html#" << anchorStr << "\">" << lineNr[i] << "</a>";
        }
        anchorStr[0]='\0';
        qsnprintf(anchorStr, maxAnchorStrLen, "l%05d", lineNr.back());
        ss << " and ";
    ...
    ss << " of file <a class=\"el\" href=\"" << sourceFileName << ".html\">" << fileName << "</a>.";
  } else {
    if (size == 1) {
      ss << "Used at line ";
      ss << lineNr.front();
      ...
      ss << "Used at lines ";
      if (size == 2) {
        ss << lineNr.front() << " and " << lineNr.back();
      } else {
        ss << lineNr.front();
        for (int i = 1; i < size - 1; i++) {
          ss << ", " << lineNr[i];
