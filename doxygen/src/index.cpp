...
void addFileMemberNameToIndex(const MemberDef *md)
{
  ...
  if (fd && fd->isLinkableInProject() && md->isLinkableInProject())
  {
    ...
    if (!letter.empty())
    {
      ...
      else if (md->isKconfig()) /** @author Jobayer Ahmmed */
      {
        MemberIndexMap_add(g_fileIndexLetterUsed[FMHL_Kconfigs],letter,md);
        documentedFileMembers[FMHL_Kconfigs]++;
      }
      else if (md->isDefine())
      {
...
static const FmhlInfo *getFmhlInfo(size_t hl)
{
  ...
  static FmhlInfo fmhlInfo[] =
  {
    ...
    /** Follow order of FileMemberHighlight @author Jobayer Ahmmed */
    FmhlInfo("globals_kconfs",QCString("Kconfigs")),
...
static void writeFileMemberIndex(OutputList &ol)
{
  ...
  writeFileMemberIndexFiltered(ol,FMHL_Kconfigs); /** @author Jobayer Ahmmed */
...
static void writeIndex(OutputList &ol)
{
  ...
  /**
   * If there is no comment for the landing page (index.html)
   * in any of the source files, then the landing page is empty.
   * In that case, the Doxygen::mainPage is null and by checking that
   * we are showing a simple message in the landing page.
   * 
   * @author Jobayer Ahmmed
   */
  else {
    QCString fileName = "index";
    QCString doc = "Please select a file from <B>Files</B> menu.";
    ol.startTextBlock();
    ol.generateDoc(fileName, -1, 0, 0,
                doc, TRUE, FALSE,
                QCString(), FALSE, FALSE, Config_getBool(MARKDOWN_SUPPORT));
    ol.endTextBlock();
  }
