...
/**
 * Represents a kconfig used as an ifdef macro in source code.
...
/** List of all ifdef kconfigs */
using KconfigList = std::vector<Kconfig>;
using KconfigsPerFileList = std::unordered_map<std::string, KconfigList>;

/**
 * Represents a kconfig definition defined in kconfig files.
 * @author Jobayer Ahmmed
 */
class KconfigInfo 
{
  public:
    ...
    std::vector<std::pair<QCString, QCString>> defaults;
    std::vector<std::pair<QCString, QCString>> selects;
    std::vector<std::pair<QCString, QCString>> implies;
    std::vector<std::pair<QCString, QCString>> ranges;

    KconfigInfo(QCString m_name) : name(m_name) {}

    KconfigInfo(
      QCString m_name,
      QCString m_type,
      QCString m_dependsOn,
      QCString m_help,
      QCString m_filepath,
      int m_lineno,
      std::pair<QCString, QCString> m_prompt,
      std::vector<std::pair<QCString, QCString>> m_defaults,
      std::vector<std::pair<QCString, QCString>> m_selects,
      std::vector<std::pair<QCString, QCString>> m_implies,
      std::vector<std::pair<QCString, QCString>> m_ranges
    ) : 
    name(m_name),
    type(m_type),
...
class Doxygen
{
  public:
    ...
    /** 
     * Stores all kconfigs extracted from all the kconfig files (i.e. Config.in).
     * @author Jobayer Ahmmed 
     */
    static std::unordered_map<std::string, KconfigInfo> kconfigDict;

    /**
     * Holds all ifdef kconfig macros used in all the source code files.
     * These are the kconfigs used as ifdef preprocessors in source code.
     * 
     * @author Jobayer Ahmmed
     */
    static KconfigsPerFileList       kconfigMacros;
};
