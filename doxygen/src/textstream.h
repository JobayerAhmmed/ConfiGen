...
class TextStream final
{
  ...
  public:
   ...
   ~TextStream() { flush(); }
    ...
    /**
     * Removes last \a n characters from string.
     * @param n number of characters to be removed from end
     * 
     * @author Jobayer Ahmmed
     */
    void erase_from_end(const int n)
    {
      m_buffer.erase(m_buffer.size() - n);
    }
