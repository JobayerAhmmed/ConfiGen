...
/** @author Jobayer Ahmmed */
std::unordered_map<std::string, KconfigInfo> Doxygen::kconfigDict;
KconfigsPerFileList   Doxygen::kconfigMacros;
...
/**
 * Add the used Kconfigs found by cscope as file members
 * with their documentation.
 * 
 * @author Jobayer Ahmmed
 */
static void setKconfigDocumentation()
{
  ...
  if (getcwd(cwd, sizeof(cwd)) == nullptr) {
    err("Error getting current working directory! Exiting...\n");
    exit(1);
  }
  QCString dirPrefix = QCString(cwd) + separator;

  const StringVector &macroPrefixes = Config_getList(KCONFIG_MACRO_PREFIX);
  const StringVector &macroSuffixes = Config_getList(KCONFIG_MACRO_SUFFIX);

  QCString fileName;
  for (const auto &s : g_inputFiles) {
    // Remove cwd prefix from each file name
    fileName = QCString(s);
    fileName.stripPrefix(dirPrefix);
    auto it = Doxygen::kconfigMacros.find(fileName.str());
    if (it!=Doxygen::kconfigMacros.end()) {
      for (auto &def : it->second) {
        QCString name = def.name;
        for (const auto &prefix : macroPrefixes) {
          QCString p = QCString(prefix);
          if (name.startsWith(p)) {
            name.stripPrefix(p);
            break;
          }
        }
        for (const auto &suffix : macroSuffixes) {
          QCString suff = QCString(suffix);
          if (name.endsWith(suff)) {
            name = name.left(name.size() - suff.size());
            break;
          }
        }

        auto kconIt = Doxygen::kconfigDict.find(name.str());
        if (kconIt != Doxygen::kconfigDict.end()) {
          bool ambig;
          FileDef* fd = findFileDef(Doxygen::inputNameLinkedMap, QCString(s), ambig);
          if (fd && !ambig) {
            def.fileDef = fd;
          }
          std::unique_ptr<MemberDefMutable> md { createMemberDef(
              def.fileName, def.lineNr.front(), 1,
              "kconfig", def.name, "",
              QCString(), Public, Normal, FALSE, 
              Member, MemberType_Kconfig, ArgumentList(), 
              ArgumentList(), "") };
          md->setFileDef(def.fileDef);

          std::stringstream ss;
          ss << "<span data-kconfig=\"definition\">";
          ss << "<b>Name:</b> " << name << "<br>";
          ss << "<b>Type:</b> " << kconIt->second.type << "<br>";

          if (kconIt->second.defaults.size() > 0) {
            ss << "<b>Default:</b> ";
            for (auto& item : kconIt->second.defaults) {
              if (item.second.isEmpty()) {
                ss << item.first << "<br>";
              } else {
                ss << item.first << " <b>if</b> " << item.second << "<br>";
              }
            }
          }

          if (kconIt->second.prompt.first != NULL) {
            ss << "<b>Prompt:</b> " << kconIt->second.prompt.first;
            if (!kconIt->second.prompt.second.isEmpty())
              ss << " <b>if</b> " << kconIt->second.prompt.second;
            ss << "<br>";
          }

          if (kconIt->second.dependsOn != NULL) {
            ss << "<b>Depends on:</b> " << kconIt->second.dependsOn << "<br>";
          }
          
          if (kconIt->second.selects.size() > 0) {
            ss << "<b>Select:</b> ";
            for (auto& item : kconIt->second.selects) {
              if (item.second.isEmpty()) {
                ss << item.first << "<br>";
              } else {
                ss << item.first << " <b>if</b> " << item.second << "<br>";
          ...
          if (kconIt->second.implies.size() > 0) {
            ss << "<b>Imply:</b> ";
            for (auto& item : kconIt->second.implies) {
              if (item.second.isEmpty()) {
                ss << item.first << "<br>";
              } else {
                ss << item.first << " <b>if</b> " << item.second << "<br>";
          ...
          if (kconIt->second.ranges.size() > 0) {
            ss << "<b>Range:</b> ";
            for (auto& item : kconIt->second.ranges) {
              if (item.second.isEmpty()) {
                ss << item.first << "<br>";
              } else {
                ss << item.first << " <b>if</b> " << item.second << "<br>";
          ...
          if (kconIt->second.help != NULL) {
            ss << "<b>Help:</b>";
            ss << "</span>";
            ss << "<pre style=\"font-family: var(--font-family-normal); margin: -10px 0 0 18px;\" data-kconfig=\"help\">"
              << kconIt->second.help.stripWhiteSpace() << "</pre>";
          }
          else {
            ss << "</span>";
          }
          md->setDocumentation(QCString(ss.str()), def.fileName, def.lineNr.front());

          ss.str(std::string());
          if (kconIt->second.filepath != NULL) {
            ss << "<span data-kconfig=\"defined-at\">";
            ss << "Defined at line " << kconIt->second.lineno << " of file " << kconIt->second.filepath << ".";
            ss << "</span>";
          }
          md->setDocumentation(QCString(ss.str()), def.fileName, -1);

          ss.str(std::string());
          ss << "<span data-kconfig=\"used-at\">";
          if (def.fileDef) {
            ss << getUsedAt(
              def.fileDef->getSourceFileBase(),
              def.fileName,
              def.lineNr, Config_getBool(SOURCE_BROWSER)).str();
          } else {
            ss << getUsedAt(
              QCString(),
              def.fileName,
...
/**
 * Print a Kconfig defintion.
 * @author Jobayer Ahmmed
 */
// static void printKconfigInfo(KconfigInfo kdef)
// {
//   std::stringstream ss;
//   ss << "config ";
...
//   ss << kdef.name.data() << "\n";
//   ss << "\t" << kdef.type.data();
//   if (kdef.prompt.first != NULL) {
//     ss << " " << kdef.prompt.first.data();
//     if (!kdef.prompt.second.isEmpty()) {
//       ss << " if " << kdef.prompt.second.data();
//     }
//   }
//   ss << "\n";
//   if (kdef.defaults.size() > 0) {
//     for (auto& item : kdef.defaults) {
//       ss << "\tdefault " << item.first.data();
//       if (!item.second.isEmpty()) {
//         ss << " if " << item.second.data();
//       }
//       ss << "\n";
//     }
//   }
//   if (kdef.dependsOn != NULL) {
//     ss << "\tdepends on " << kdef.dependsOn.data() << "\n";
//   }
//   if (kdef.selects.size() > 0) {
//     for (auto& item : kdef.selects) {
//       ss << "\tselect " << item.first.data();
//       if (!item.second.isEmpty()) {
//         ss << " if " << item.second.data();
//       }
//       ss << "\n";
//     }
//   }
//   if (kdef.implies.size() > 0) {
//     for (auto& item : kdef.implies) {
//       ss << "\timply " << item.first.data();
//       if (!item.second.isEmpty()) {
//         ss << " if " << item.second.data();
//       }
//       ss << "\n";
//     }
//   }
//   if (kdef.ranges.size() > 0) {
//     for (auto& item : kdef.ranges) {
//       ss << "\trange " << item.first.data();
//       if (!item.second.isEmpty()) {
//         ss << " if " << item.second.data();
//       }
//       ss << "\n";
...
//   }
//   if (kdef.help != NULL) {
//     ss << "\thelp\n\t  " << kdef.help.stripWhiteSpace().data() << "\n";
//   }
//   if (kdef.filepath != NULL) {
//     ss << "\tDefined at line " << kdef.lineno << " of file " << kdef.filepath.data() << ".\n";
//   }
//   QCString s = QCString(ss.str());
//   msg("%s", s.data());
// }

/**
 * Create an xml file that contains all the kconfig definitions 
 * extracted from Config.in/src files.
 * 
 * @author Jobayer Ahmmed
 */
static void dumpKconfigDefs()
{
  QCString outputDirectory = Config_getString(OUTPUT_DIRECTORY);
  QCString usageDirectory = Config_getString(HTML_OUTPUT) + "/kconfig";

  Dir formatDir(usageDirectory.str());
  if (!formatDir.exists() && !formatDir.mkdir(usageDirectory.str())) {
    err("Could not create kconfig directory %s\n", qPrint(usageDirectory));
    cleanUpDoxygen();
    exit(1);
  }

  // Create ordered map of Kconfigs per Kconfig file
  std::map<std::string, std::vector<KconfigInfo>> kconfigsPerFile;
  for (const auto &kdef : Doxygen::kconfigDict) {
    auto fileItem = kconfigsPerFile.find(kdef.second.filepath.str());
    if (fileItem == kconfigsPerFile.end()) {
      std::vector<KconfigInfo> kconfigList;
      kconfigList.push_back(kdef.second);
      kconfigsPerFile.insert(std::make_pair(kdef.second.filepath.str(), kconfigList));
      kconfigList.clear();
    } else {
      fileItem->second.push_back(kdef.second);
    }
  }

  // Sort Kconfig defs for each file
  for (auto &item : kconfigsPerFile) {
    std::sort(item.second.begin(), item.second.end(), [](const KconfigInfo &a, const KconfigInfo &b) {
      return a.name.str() < b.name.str();
    });
  }

  pugi::xml_document doc;
  pugi::xml_node root = doc.append_child("KconfigRoot");
  
  for (const auto &kdefPerFile : kconfigsPerFile) {
    pugi::xml_node file = root.append_child("file");
    file.append_attribute("fileName").set_value(kdefPerFile.first.c_str());
    for (const auto &kdef : kdefPerFile.second) {
      pugi::xml_node kconfig = file.append_child("Kconfig");
      kconfig.append_attribute("name").set_value(kdef.name.data());
      kconfig.append_child("type").text().set(kdef.type.data());
    
      if (kdef.prompt.first != NULL) {
        pugi::xml_node node = kconfig.append_child("prompt");
        node.text().set(kdef.prompt.first.data());
        pugi::xml_attribute depends = node.append_attribute("depends");
        if (!kdef.prompt.second.isEmpty()) {
          depends.set_value(kdef.prompt.second.data());
        }
      }
      if (kdef.defaults.size() > 0) {
        for (auto& item : kdef.defaults) {
          pugi::xml_node node = kconfig.append_child("default");
          node.text().set(item.first.data());
          pugi::xml_attribute depends = node.append_attribute("depends");
          if (!item.second.isEmpty()) {
            depends.set_value(item.second.data());
          }
        }
      }
      if (kdef.dependsOn != NULL) {
        kconfig.append_child("depends").text().set(kdef.dependsOn.data());
      }
      if (kdef.selects.size() > 0) {
        for (auto& item : kdef.selects) {
          pugi::xml_node node = kconfig.append_child("select");
          node.text().set(item.first.data());
          pugi::xml_attribute depends = node.append_attribute("depends");
          if (!item.second.isEmpty()) {
            depends.set_value(item.second.data());
      ...
      if (kdef.implies.size() > 0) {
        for (auto& item : kdef.implies) {
          pugi::xml_node node = kconfig.append_child("imply");
          node.text().set(item.first.data());
          pugi::xml_attribute depends = node.append_attribute("depends");
          if (!item.second.isEmpty()) {
            depends.set_value(item.second.data());
      ...
      if (kdef.ranges.size() > 0) {
        for (auto& item : kdef.ranges) {
          pugi::xml_node node = kconfig.append_child("range");
          node.text().set(item.first.data());
          pugi::xml_attribute depends = node.append_attribute("depends");
          if (!item.second.isEmpty()) {
            depends.set_value(item.second.data());
      ...
      if (kdef.help != NULL) {
        kconfig.append_child("help").text().set(kdef.help.stripWhiteSpace().data());
      }
      if (kdef.filepath != NULL) {
        kconfig.append_child("filepath").text().set(kdef.filepath.data());
        kconfig.append_child("lineno").text().set(kdef.lineno);
      }
    }
  }

  QCString filepath = usageDirectory + "/kconfig_defs.xml";
  bool save_status = doc.save_file(filepath.data());
  if (!save_status) {
    err("Could not create Kconfig definition file %s\n", qPrint(filepath));
    cleanUpDoxygen();
    exit(1);
  }
}
...
/**
 * Create an xml file that contains kconfig #ifdef macro usage
 * in C source files.
 * 
 * @author Jobayer Ahmmed
 */
static void dumpKconfigUsage()
{
  QCString outputDirectory = Config_getString(OUTPUT_DIRECTORY);
  QCString usageDirectory = Config_getString(HTML_OUTPUT) + "/kconfig";
  ...
  QCString projectPath = Dir::currentDirPath();
  size_t projectPathLen = projectPath.size() + 1;

  // Sort Kconfig macro usage for each file
  std::map<std::string, KconfigList> orderedKconfigMacros(Doxygen::kconfigMacros.begin(), Doxygen::kconfigMacros.end());
  for (auto &item : orderedKconfigMacros) {
    std::sort(item.second.begin(), item.second.end(), [](const Kconfig &a, const Kconfig &b) {
      return a.name.str() < b.name.str();
    });
  }
  ...
  for (const auto &item : orderedKconfigMacros) {
    if (item.second.size() < 1) {
      continue;
    }
    pugi::xml_node file = root.append_child("file");
    pugi::xml_attribute fileName = file.append_attribute("fileName");
    if (projectPathLen < item.first.size()) {
      fileName.set_value(item.first.substr(projectPathLen).c_str());
    } else {
      fileName.set_value(item.first.c_str());
    }
    for (const auto &kconf : item.second) {
      pugi::xml_node kconfig = file.append_child("Kconfig");
      kconfig.append_attribute("name").set_value(kconf.name.data());
      for (const auto &line : kconf.lineNr) {
        kconfig.append_child("line").text().set(line);
      }
    }
  }

  QCString filepath = usageDirectory + "/kconfig_usage.xml";
  bool save_status = doc.save_file(filepath.data());
  if (!save_status) {
    err("Could not create Kconfig usage file %s\n", qPrint(filepath));
    cleanUpDoxygen();
    exit(1);
  }
}
...
/**
 * Reads a line of variable size.
 * @author Jobayer Ahmmed
 */
char* kxtGetLine(FILE* fp) {
  ...
  do {
    ...
    char *extra = (char*) realloc(line, buf_len + cur_len + 1);
    if (extra == 0)
      return line;
    line = extra;
    strcpy(line + cur_len, buffer);
    cur_len += buf_len;
...
/**
 * Runs kextractor of kmax toolsuit and extracts all kconfig info.
 * @author Jobayer Ahmmed
 */
void runKextractor() {
  const QCString PREFIX_CONFIG    =   "config ";
  const QCString PREFIX_PROMPT    =   "prompt ";
  const QCString PREFIX_DEFAULT   =   "def ";
  const QCString PREFIX_DEPENDSON =   "dep ";
  const QCString PREFIX_SELECT    =   "select ";
  const QCString PREFIX_IMPLY     =   "imply ";
  const QCString PREFIX_RANGE     =   "range ";
  const QCString PREFIX_HELP      =   "help ";
  const QCString PREFIX_LINE      =   "line ";
  const QCString PREFIX_FILEPATH  =   "file ";

  QCString entryFile = Config_getString(KCONFIG_ENTRY_FILE);
  msg("Running kextractor for file '%s'...\n", entryFile.data());

  QCString cmd = "kextract -p --extractall " + entryFile;
  FILE* fp = Portable::popen(cmd, "r");
  if (!fp) {
    err("Error running kextract! Exiting...\n");
    exit(1);
  }
  ...
  char* line = kxtGetLine(fp);

  while (line != NULL) {
    QCString qcs = QCString(line);
    // remove \n from end
    if (qcs.size() > 0) {
      qcs.remove(qcs.size()-1, 1);
    }
    if (qcs.startsWith(PREFIX_CONFIG)) { // name and type
      qcs.stripPrefix(PREFIX_CONFIG);
      index1 = qcs.find(' ');
      if (index1 != -1) {
        name = qcs.left(index1);
        QCString type = qcs.mid(index1 + 1);
        auto item = Doxygen::kconfigDict.find(name.str());
        if (item == Doxygen::kconfigDict.end()) {
          KconfigInfo kcon = KconfigInfo(name);
          kcon.type = type;
          Doxygen::kconfigDict.insert(std::make_pair(name.str(), kcon));
        } else {
          item->second.type = type;
        }
      }
    } else if (qcs.startsWith(PREFIX_PROMPT)) { // prompt
      std::pair<QCString, QCString> prompt;
      qcs.stripPrefix(PREFIX_PROMPT);
      index1 = qcs.find(' ');
      if (index1 != -1) {
        name = qcs.left(index1);
        index2 = qcs.find(" |(", index1);
        if (index2 == -1) {
          prompt = std::make_pair(qcs.mid(index1 + 1), QCString());
        } else {
          prompt = std::make_pair(qcs.mid(index1 + 1, index2 - index1 - 1), qcs.mid(index2 + 2));
        }
        if (prompt.second == "(1)") {
          prompt.second = QCString();
        }
        auto item = Doxygen::kconfigDict.find(name.str());
        if (item == Doxygen::kconfigDict.end()) {
          KconfigInfo kcon = KconfigInfo(name);
          ...
          item->second.prompt = prompt;
        }
      }
    } else if (qcs.startsWith(PREFIX_DEFAULT)) { // defaults
      std::pair<QCString, QCString> defaults;
      qcs.stripPrefix(PREFIX_DEFAULT);
      index1 = qcs.find(' ');
      if (index1 != -1) {
        name = qcs.left(index1);
        index2 = qcs.find(" |(", index1);
        if (index2 == -1) {
          defaults = std::make_pair(qcs.mid(index1 + 1), QCString());
        } else {
          defaults = std::make_pair(qcs.mid(index1 + 1, index2 - index1 - 1), qcs.mid(index2 + 2));
        }
        if (defaults.second == "(1)") {
          defaults.second = QCString();
        }
        auto item = Doxygen::kconfigDict.find(name.str());
        if (item == Doxygen::kconfigDict.end()) {
          KconfigInfo kcon = KconfigInfo(name);
          kcon.defaults.push_back(defaults);
          Doxygen::kconfigDict.insert(std::make_pair(name.str(), kcon));
        } else {
          item->second.defaults.push_back(defaults);
        }
      }
    } else if (qcs.startsWith(PREFIX_DEPENDSON)) { // depends on
      qcs.stripPrefix(PREFIX_DEPENDSON);
      index1 = qcs.find(' ');
      if (index1 != -1) {
        name = qcs.left(index1);
        QCString dependsOn = qcs.mid(index1 + 1);
        if (dependsOn != "(1)") {
          auto item = Doxygen::kconfigDict.find(name.str());
          if (item == Doxygen::kconfigDict.end()) {
            KconfigInfo kcon = KconfigInfo(name);
            kcon.dependsOn = dependsOn;
            Doxygen::kconfigDict.insert(std::make_pair(name.str(), kcon));
          } else {
            if (item->second.dependsOn.isEmpty()) {
              item->second.dependsOn = dependsOn;
            } else {
              item->second.dependsOn = item->second.dependsOn + " and " + dependsOn;
            }
          }
        }
      }
    } else if (qcs.startsWith(PREFIX_SELECT)) { // selects
      std::pair<QCString, QCString> selects;
      qcs.stripPrefix(PREFIX_SELECT);
      index1 = qcs.find(' ');
      if (index1 != -1) {
        QCString selected = qcs.left(index1);
        index2 = qcs.find(" |(", index1);
        if (index2 == -1) {
          name = qcs.mid(index1 + 1);
          selects = std::make_pair(selected, QCString());
        } else {
          name = qcs.mid(index1 + 1, index2 - index1 - 1);
          selects = std::make_pair(selected, qcs.mid(index2 + 2));
        }
        if (selects.second == "(1)") {
          selects.second = QCString();
        }
        auto item = Doxygen::kconfigDict.find(name.str());
        if (item == Doxygen::kconfigDict.end()) {
          KconfigInfo kcon = KconfigInfo(name);
          kcon.selects.push_back(selects);
          Doxygen::kconfigDict.insert(std::make_pair(name.str(), kcon));
        } else {
          item->second.selects.push_back(selects);
        }
      }
    } else if (qcs.startsWith(PREFIX_IMPLY)) { // implies
      std::pair<QCString, QCString> implies;
      qcs.stripPrefix(PREFIX_IMPLY);
      index1 = qcs.find(' ');
      if (index1 != -1) {
        QCString selected = qcs.left(index1);
        index2 = qcs.find(" |(", index1);
        if (index2 == -1) {
          name = qcs.mid(index1 + 1);
          implies = std::make_pair(selected, QCString());
        } else {
          name = qcs.mid(index1 + 1, index2 - index1 - 1);
          implies = std::make_pair(selected, qcs.mid(index2 + 2));
        }
        if (implies.second == "(1)") {
          implies.second = QCString();
        }
        auto item = Doxygen::kconfigDict.find(name.str());
        if (item == Doxygen::kconfigDict.end()) {
          KconfigInfo kcon = KconfigInfo(name);
          kcon.implies.push_back(implies);
          Doxygen::kconfigDict.insert(std::make_pair(name.str(), kcon));
        } else {
          item->second.implies.push_back(implies);
        }
      }
    } else if (qcs.startsWith(PREFIX_RANGE)) { // ranges
      std::pair<QCString, QCString> ranges;
      qcs.stripPrefix(PREFIX_RANGE);
      index1 = qcs.find(' ');
      if (index1 != -1) {
        name = qcs.left(index1);
        index2 = qcs.find(" |(", index1);
        if (index2 == -1) {
          ranges = std::make_pair(qcs.mid(index1 + 1), QCString());
        } else {
          ranges = std::make_pair(qcs.mid(index1 + 1, index2 - index1 - 1), qcs.mid(index2 + 2));
        }
        if (ranges.second == "(1)") {
          ranges.second = QCString();
        }
        auto item = Doxygen::kconfigDict.find(name.str());
        if (item == Doxygen::kconfigDict.end()) {
          KconfigInfo kcon = KconfigInfo(name);
          kcon.ranges.push_back(ranges);
          Doxygen::kconfigDict.insert(std::make_pair(name.str(), kcon));
        } else {
          item->second.ranges.push_back(ranges);
        }
      }
    } else if (qcs.startsWith(PREFIX_LINE)) { // lineno
      qcs.stripPrefix(PREFIX_LINE);
      index1 = qcs.find(' ');
      if (index1 != -1) {
        name = qcs.left(index1);
        QCString lineno = qcs.mid(index1 + 1);
        auto item = Doxygen::kconfigDict.find(name.str());
        if (item == Doxygen::kconfigDict.end()) {
          KconfigInfo kcon = KconfigInfo(name);
          kcon.lineno = std::stoi(lineno.str());
          Doxygen::kconfigDict.insert(std::make_pair(name.str(), kcon));
        } else {
          item->second.lineno = std::stoi(lineno.str());
        }
      }
    } else if (qcs.startsWith(PREFIX_FILEPATH)) { // filepath
      qcs.stripPrefix(PREFIX_FILEPATH);
      index1 = qcs.find(' ');
      if (index1 != -1) {
        name = qcs.left(index1);
        QCString filepath = qcs.mid(index1 + 1);
        // remove double quote
        if (filepath.size() > 2) {
          filepath.remove(0, 1);
        ...
        if (item == Doxygen::kconfigDict.end()) {
          KconfigInfo kcon = KconfigInfo(name);
          kcon.filepath = filepath;
          Doxygen::kconfigDict.insert(std::make_pair(name.str(), kcon));
        } else {
          item->second.filepath = filepath;
        }
      }
    } else if (qcs.startsWith(PREFIX_HELP)) { // help
      qcs.stripPrefix(PREFIX_HELP);
      index1 = qcs.find(' ');
      if (index1 != -1) {
        name = qcs.left(index1);
        QCString help = qcs.mid(index1 + 1);
        // remove double quote
        if (help.size() > 2) {
          help.remove(0, 1);
          help.remove(help.size() - 1, 1);
        }
        // replace <newline> with \n
        help = substitute(help, "<newline>", "\n");
        auto item = Doxygen::kconfigDict.find(name.str());
        if (item == Doxygen::kconfigDict.end()) {
          KconfigInfo kcon = KconfigInfo(name);
          kcon.help = help;
  ...
  int status = Portable::pclose(fp);
  if (status != 0) {
    err("Error reading file %s! Exiting...\n", entryFile.data());
    exit(1);
  }
}

static void searchSymbols(std::unordered_map<std::string, std::vector<QCString>> &searchResult,
                          std::pair<QCString, QCString> pair) {
  QCString psSymbol;
  QCString cmd;
  FILE* fp;
  char* line;
  ...
  for (const auto &kdef : Doxygen::kconfigDict) {
    std::vector<QCString> lines;
    psSymbol = pair.first + kdef.second.name + pair.second;
    cmd = QCString("cscope -d -L -0\"") + psSymbol + QCString("\"");
    fp = Portable::popen(cmd, "r");
    if (!fp) {
      err("Could not search symbol %s by cscope!\n", psSymbol.data());
    } else {
      line = kxtGetLine(fp);
      while (line != NULL) {
        qcs = QCString(line);
    ...
    searchResult.emplace(std::make_pair(psSymbol.str(), std::move(lines)));
    status = Portable::pclose(fp);
    if (status != 0) {
      err("Error closing FILE* in cscope search for %s!\n", psSymbol.data());
    }
  }
}

/**
 * Runs cscope and extracts all kconfig usage info.
 * @author Jobayer Ahmmed
 */
void runCscope() {
  msg("Running cscope ...\n");
  ...
  char cwd[1024];
  if (getcwd(cwd, sizeof(cwd)) == nullptr) {
    err("Error getting current working directory for cscope! Exiting...\n");
    exit(1);
  }
  ...
  // Create cscope.files with file names
  // that match file patterns from current directory.
  const StringVector &filePatterns = Config_getList(FILE_PATTERNS);
  QCString fileStr;
  for (size_t i = 0; i < filePatterns.size(); ++i) {
    fileStr += QCString("-name '") + filePatterns[i].c_str() + QCString("'");
    if (i != filePatterns.size() - 1) {
      fileStr += QCString(" -o ");
    }
  }
  QCString cscopeFile = QCString(cwd) + separator + QCString("cscope.files");
  cmd = QCString("find ") + QCString(cwd) + QCString(" -type f \\( ")
    + fileStr + QCString(" \\)");
  fp = Portable::popen(cmd, "r");
  if (!fp) {
    err("Error creating cscope input file! Exiting...\n");
    exit(1);
  } else {
    FILE* outFile = fopen(cscopeFile.data(), "w");
    line = kxtGetLine(fp);
    while (line != NULL) {
      qcs = QCString(line);
      // remove \n from end
  ...
  status = Portable::pclose(fp);
  if (status != 0) {
    err("Error closing FILE* in cscope input file! Exiting...\n");
    exit(1);
  }

  // Create cscope db by reading the filenames.
  cmd = QCString("cscope -q -R -b -i") + cscopeFile;
  fp = Portable::popen(cmd, "r");
  if (!fp) {
    err("Error creating cscope db! Exiting...\n");
    exit(1);
  }
  status = Portable::pclose(fp);
  if (status != 0) {
    err("Error closing FILE* in cscope db build! Exiting...\n");
    exit(1);
  }

  // Search Kconfig symbol usage
  const StringVector &macroPrefixes = Config_getList(KCONFIG_MACRO_PREFIX);
  const StringVector &macroSuffixes = Config_getList(KCONFIG_MACRO_SUFFIX);

  // Make prefix suffix pairs.
  // Assumming prefix has >= items than suffix.
  std::vector<std::pair<QCString, QCString>> pairs;
  for (size_t i = 0; i < macroPrefixes.size(); i++) {
    if (i < macroSuffixes.size()) {
      pairs.push_back(std::make_pair(macroPrefixes[i], macroSuffixes[i]));
    } else {
      pairs.push_back(std::make_pair(macroPrefixes[i], QCString()));
    }
  }

  // Search result lines per kdef
  std::unordered_map<std::string, std::vector<QCString>> searchResult;
  if (pairs.size() > 0) {
    for (const auto& pair : pairs) {
      searchSymbols(searchResult, pair);
    }
  } else {
    searchSymbols(searchResult, std::make_pair(QCString(), QCString()));
  }

  // Remove cwd prefix from each line item
  QCString dirPrefix = QCString(cwd) + separator;
  for (auto& pair : searchResult) {
    for (auto& item : pair.second) {
      item.stripPrefix(dirPrefix);
    }
  }

  // Process search result
  // <filename, <kdef_name, Kconfig>
  std::map<std::string, std::map<std::string, Kconfig>> fileKconfigMap;
  QCString fileName, lineNr;
  int spaceIndex1, spaceIndex2, spaceIndex3;
  int lineno;
  for (auto& resItem : searchResult) {
    for (auto& item : resItem.second) {
      // Extract fileName and lineNr
      // busybox/editors/vi.c <global> 396 char ...
      // busybox/editors/vi.c undo_push 2327 undo_queue...
      spaceIndex1 = item.find(' ');
      if (spaceIndex1 != -1) {
        fileName = item.left(spaceIndex1);
        spaceIndex2 = item.find(' ', spaceIndex1 + 1);
        if (spaceIndex2 != -1) {
          spaceIndex3 = item.find(' ', spaceIndex2 + 1);
          if (spaceIndex3 != -1) {
            lineNr = item.mid(spaceIndex2 + 1, spaceIndex3 - spaceIndex2 - 1);
            try {
              lineno = std::stoi(lineNr.str());
            } catch (...) {
              lineno = -1;
              err("Error in converting string [%s] to int\n", lineNr.data());
            }
            if (lineno == -1) {
              continue;
            }
            ...
            Kconfig kconf;
            kconf.name = QCString(resItem.first);
            kconf.fileName = fileName;
            kconf.lineNr.push_back(lineno);

            auto fileIt = fileKconfigMap.find(fileName.str());
            if (fileIt != fileKconfigMap.end()) {
              auto kdefIt = fileIt->second.find(resItem.first);
              if (kdefIt != fileIt->second.end()) {
                kdefIt->second.lineNr.push_back(lineno);
              } else {
                fileIt->second.insert(std::make_pair(resItem.first, kconf));
              }
            } else {
              fileKconfigMap[fileName.str()].insert(std::make_pair(resItem.first, kconf));
            }
          }
        }
      }
  ...
  // Store in Doxygen::kconfigMacros
  for (auto& fileEntry : fileKconfigMap) {
    KconfigList kconfigList;
    for (auto& kdefMap : fileEntry.second) {
      kconfigList.push_back(kdefMap.second);
    }
    Doxygen::kconfigMacros.emplace(std::make_pair(fileEntry.first,
      std::move(kconfigList)));
  }

  // Delete cscope files
  std::vector<QCString> cscopeFiles;
  cscopeFiles.push_back(dirPrefix + QCString("cscope.files"));
  cscopeFiles.push_back(dirPrefix + QCString("cscope.in.out"));
  cscopeFiles.push_back(dirPrefix + QCString("cscope.out"));
  cscopeFiles.push_back(dirPrefix + QCString("cscope.po.out"));
  Dir thisDir;
  for (QCString file : cscopeFiles) {
    thisDir.remove(file.str());
  }
}
...
void parseInput()
{
  ...
  /**
   * Read kconfig source and macro files
   * @author Jobayer Ahmmed
   */
  runKextractor();
  runCscope();
   ...
   * @author Jobayer Ahmmed
   */
  g_s.begin("Setting Kconfig documentation...\n");
  setKconfigDocumentation();
  g_s.end();
...
void generateOutput()
{
  bool generateDoc = Config_getBool(GENERATE_DOC);
  if (generateDoc) {
    /**************************************************************************
   ...
   * @author Jobayer Ahmmed
   */
  g_s.begin("Dumping Kconfig definitions to kconfig_defs.xml file...\n");
  dumpKconfigDefs();
  g_s.end();

  g_s.begin("Dumping Kconfig usage to kconfig_usage.xml file...\n");
  dumpKconfigUsage();
  g_s.end();
