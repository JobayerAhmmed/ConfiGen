...
repo_url = 'https://github.com/landley/toybox.git'
repo_branch = 'master'

project_dir_name = 'toybox'
project_name = 'Toybox'
...
project_doxyfile = os.path.join(config.root_dir, 'Doxyfile_toybox')
project_gen_config_file = os.path.join(config.root_dir, 'gen_config_files_toybox.sh')
...
# From commit 5257cf54a5810105bc4a75703a06740d756d8e87, 2006-11-01
#   Kconfig was added in Toybox.
# From commit 2896480c4918f2accccb8301bec457a7bff7377e, 2008-01-19
#   Kconfig defs were defined in *.c files
#   and scripts/genconfig.sh was introduced.
#   Checked by code, location of genconfig.sh file has not been changed
#   since introduced and remained at scripts/genconfig.sh.
# From commit 1b7ad01f5e5ead83fbc06ff20248b085a15dc279, 2012-02-24
#   source generated/Config.probed was added in Config.in file.
#   For this, probeconfig() was added in scripts/genconfig.sh file.
#   We will not consider Config.probed because it only contains hardcoded
#   two Kconfig definition generated by the genconf.sh file.
#   For that, we need to comment out source generated/Config.probed
#   from Config.in file.
# For commit id 9317c06e2d2081feb37ab36e626707f3a1205576, v0.4.0,
#   Kconfig prefix is: CFG_
#   ref: toys/dmesg.c:57

# First commit, 2006-09-27
start_commit_id = '13bab2f09e9189c2b1e83ae37a1134108f9479cf'
# Last commit of 2023, 2023-12-29
end_commit_id = 'a17fee77885125974adf3499f23d6de8835d0f38'
# 2012-03-07
config_prefix_start_commit_id = 'ed6ed62f7db42ee72532fbc848b2d505e81bef39'
# 2006-11-01
config_start_commit_id = '5257cf54a5810105bc4a75703a06740d756d8e87'

kdef_start = re.compile('^(?:[ \t]*config[ \t]+)([A-Z][A-Za-z0-9_]*)[ \t]*(?:#.*)?$')
kdef_add = re.compile('^(?:\+[ \t]*config[ \t]+)([A-Z][A-Za-z0-9_]*)[ \t]*(?:#.*)?$')
kdef_del = re.compile('^(?:-[ \t]*config[ \t]+)([A-Z][A-Za-z0-9_]*)[ \t]*(?:#.*)?$')
...
kdef_body = re.compile('^(?: {2,}|\t+).*|\s*$')

kdef_edit_plus = re.compile('^\+(?: {2,}|\t+).*$')
kdef_edit_minus = re.compile('^-(?: {2,}|\t+).*$')

kdef_in_src_start = kdef_start
kdef_in_src_add = kdef_add
kdef_in_src_del = kdef_del
kdef_in_src_body = kdef_body
kdef_in_src_edit_plus = kdef_edit_plus
kdef_in_src_edit_minus = kdef_edit_minus

source_file_pattern = ('*.c', '*.h')
config_file_pattern = ('Config.in',)
macro_prefixes = ('CFG_',)
macro_suffixes = ()
